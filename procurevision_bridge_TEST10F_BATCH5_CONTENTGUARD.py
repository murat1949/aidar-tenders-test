...
def diagnose_samruk_pages():
    ...
    return {
        "build":"v6.7_TEST10F_BATCH5_CONTENTGUARD",
        "safe_mode":True,
...
def auto_download_and_process_techspecs(rows):
    """
    v6.7_TEST10F_BATCH5_CONTENTGUARD:
    ...
    def do_GET(self):
        if self.path.startswith("/diagnose-pages"):
            try:
                ...
                self.headers_ok(500)
                self.wfile.write(json.dumps({"ok":False,"build":"v6.7_TEST10F_BATCH5_CONTENTGUARD","message":str(e)},ensure_ascii=False).encode("utf-8"))
...
print("ProcureVision TEST10F CONTENTGUARD local bridge: http://127.0.0.1:8765")
