...
SAMRUK_LOG=ROOT/"SAMRUK_LOG.txt"

def write_samruk_log(status, **fields):
    """TEST10O: простой локальный журнал обновлений Samruk."""
    try:
        ts=datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        parts=[f"[{ts}]", str(status)]
        for k,v in fields.items():
            if v is None:
                continue
            s=str(v).replace("\r"," ").replace("\n"," ").strip()
            parts.append(f"{k}={s}")
        with SAMRUK_LOG.open("a", encoding="utf-8") as f:
            f.write(" | ".join(parts)+"\n")
    except Exception:
        # Журнал никогда не должен ломать само обновление.
        pass
...
def samruk_user_tabs(tabs=None):
    """
    TEST10O: возвращает пользовательские вкладки Samruk.
    Если их больше одной, обновление НЕ запускаем, чтобы не выбрать чужую выдачу.
    """
    if tabs is None:
        tabs=get_tabs()
    return [
        t for t in tabs
        if "zakup.sk.kz" in (t.get("url") or "")
        and t.get("webSocketDebuggerUrl")
        and (t.get("type") in (None, "", "page"))
    ]

def ensure_single_samruk_tab(tabs=None):
    samruk_tabs=samruk_user_tabs(tabs)
    if len(samruk_tabs)>1:
        raise RuntimeError(
            "Открыто несколько вкладок Samruk. "
            "Оставьте одну вкладку с нужным поиском и повторите обновление."
        )
    return samruk_tabs
    ...
    def do_GET(self):
        if self.path.startswith("/diagnose-pages"):
            try:
                ...
                self.headers_ok(500)
                self.wfile.write(json.dumps({"ok":False,"build":"v6.10_TEST10O_LOGGING","message":str(e)},ensure_ascii=False).encode("utf-8"))
            ...
            try:
                ...
                write_samruk_log(
                    ...
                    rows=len(rows),
                    http=status,
                    techspec=auto_ok+auto_from_card,
                    techspec_existing=auto_existing,
                    errors=pending_err+auto_err
                )
                ...
                write_samruk_log("ERROR", message=str(e))
                self.headers_ok(500)
                self.wfile.write(json.dumps({"ok":False,"message":str(e)},ensure_ascii=False).encode("utf-8"))
            ...
            try:
                ...
                self.headers_ok(500)
                self.wfile.write(json.dumps({"ok":False,"message":str(e)},ensure_ascii=False).encode("utf-8"))
        elif self.path.startswith("/samruk-log"):
            try:
                lines=[]
                if SAMRUK_LOG.exists():
                    lines=SAMRUK_LOG.read_text(encoding="utf-8",errors="replace").splitlines()[-30:]
                self.headers_ok(200)
                self.wfile.write(json.dumps({
                    "ok":True,
                    "build":"v6.10_TEST10O_LOGGING",
                    "log_file":SAMRUK_LOG.name,
                    "lines":lines
                },ensure_ascii=False).encode("utf-8"))
            except Exception as e:
                self.headers_ok(500)
...
print("ProcureVision TEST10O LOGGING local bridge: http://127.0.0.1:8765")
