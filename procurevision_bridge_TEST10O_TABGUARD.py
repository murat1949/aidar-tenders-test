    ...
    def do_GET(self):
        if self.path.startswith("/diagnose-pages"):
            try:
                ...
                self.headers_ok(500)
                self.wfile.write(json.dumps({"ok":False,"build":"v6.9_TEST10O_TABGUARD","message":str(e)},ensure_ascii=False).encode("utf-8"))
...
print("ProcureVision TEST10O TAB GUARD local bridge: http://127.0.0.1:8765")
