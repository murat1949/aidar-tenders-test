...
def diagnose_samruk_pages():
    ...
    return {
        "build":"v6.7_TEST10J_LOTCARDFALLBACK",
        "safe_mode":True,
...
def auto_download_and_process_techspecs(rows):
    ...
    print("TEST10J: выбраны лоты: %s" % ", ".join(selected), flush=True)
    ...
    for idx,row in enumerate(test_rows,1):
        ...
        print("TEST10J [%s/%s] лот %s: старт" % (idx,len(test_rows),lot_id), flush=True)
        try:
            ...
            print("TEST10J [%s/%s] лот %s: popup открыт, данные карточки сняты" % (idx,len(test_rows),lot_id), flush=True)
            ...
            if found:
                ...
                print("TEST10J [%s/%s] лот %s: PDF -> JSON -> Supabase HTTP %s" % (idx,len(test_rows),lot_id,status), flush=True)
            else:
                ...
                print("TEST10J [%s/%s] лот %s: отдельной техспеки нет; карточка -> JSON -> Supabase HTTP %s" % (idx,len(test_rows),lot_id,status), flush=True)

        except Exception as e:
            item.update({"ok":False,"status":"error","message":str(e)})
            print("TEST10J [%s/%s] лот %s: ОШИБКА: %s" % (idx,len(test_rows),lot_id,str(e)), flush=True)
        finally:
            try:
        ...
        results.append(item)
        print("TEST10J [%s/%s] лот %s: статус %s" % (idx,len(test_rows),lot_id,item.get("status")), flush=True)

    print("TEST10J: пакет завершён. Результатов: %s" % len(results), flush=True)
    try: master_ws.close()
    except Exception: pass
    return results
    ...
    def do_GET(self):
        if self.path.startswith("/diagnose-pages"):
            try:
                ...
                self.headers_ok(500)
                self.wfile.write(json.dumps({"ok":False,"build":"v6.7_TEST10J_LOTCARDFALLBACK","message":str(e)},ensure_ascii=False).encode("utf-8"))
            ...
            try:
                ...
                pending=[]
                print("TEST10J: проверка старых pending PDF пропущена; запускаю только пакет новых лотов.", flush=True)
...
print("ProcureVision TEST10J LOTCARDFALLBACK local bridge: http://127.0.0.1:8765")
