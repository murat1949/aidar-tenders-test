...
def diagnose_samruk_pages():
    ...
    def wait_expected(n,target,previous_lots,timeout=15):
        ...
        while time.time()<deadline:
            ...
            # TEST10F CONTENTGUARD: Samruk SPA может уже показать новый page/диапазон,
    ...
    pages=min(pages,10)  # DIAG: максимум 10 страниц, чтобы тест оставался коротким.
    ...
    return {
        "build":"v6.7_TEST10G_BATCH5_COVERAGECHECK",
        "safe_mode":True,
...
def read_current_samruk():
    """
    TEST10F: сбор всей выдачи Samruk выполняется в ОТДЕЛЬНОЙ временной вкладке.
    ...
    try:
        ...
        def wait_page(n,prev_lots,timeout=18):
            ...
            while time.time()<deadline:
                ...
                # TEST10F CONTENTGUARD: page/"Показано" могут измениться раньше DOM карточек.
        ...
        for pass_no in range(1,4):
            prev_lots=[]
            print("Samruk TEST10F: проход %s/3, сейчас уникальных %s/%s." % (pass_no,len(collected),target), flush=True)
            for n in range(1,pages+1):
                ...
                for attempt in range(1,4):
                    ...
                    if ok: break
                if not ok:
                    print("Samruk TEST10F: страница %s/%s не подтвердилась в проходе %s; продолжаю следующий проход." % (n,pages,pass_no), flush=True)
                    prev_lots=[]
                    continue
                ...
                for c in (snap.get("cards") or []):
                    ...
                    collected[lot]=cc
                prev_lots=current
                print("Samruk TEST10F: проход %s, страница %s/%s, lot_id=%s, уникальных %s/%s." % (
        ...
        print("Samruk TEST10F: собрано %s/%s уникальных строк во временной вкладке." % (len(cards),target), flush=True)
...
def auto_download_and_process_techspecs(rows):
    """
    v6.7_TEST10G_BATCH5_COVERAGECHECK:
    CONTENTGUARD: не принимает новую страницу, пока список lot_id фактически не сменился.
    Диагностический повтор пакета до 5 лотов обрабатываются в отдельных временных вкладках Samruk.
    Для каждого лота используется номер страницы, сохранённый при сборе выдачи.
    Основная вкладка с результатами поиска не трогается, поэтому переходы
    по карточкам не должны разрушать выдачу при пакетной обработке.
    """
    tabs=get_tabs()
    tab=choose_samruk_tab(tabs)
    if not tab:
    ...
    # Убеждаемся, что исходная вкладка действительно содержит живую выдачу.
    ...
    # TEST10: контрольный пакет — до 5 лотов без raw.techspec по всей собранной выдаче.
    candidates=rows
    test_rows=[]
    for r in candidates:
    ...
    print("TEST10F: выбраны лоты для пакета: %s" % ", ".join(selected), flush=True)
    ...
    for idx,row in enumerate(test_rows,1):
        ...
        print("TEST10D [%s/%s] лот %s: старт" % (idx,len(test_rows),lot_id), flush=True)

        try:
            if tender_has_techspec(lot_id):
                item.update({"ok":True,"status":"already_has_techspec"})
                print("TEST10D [%s/%s] лот %s: уже есть techspec в Supabase" % (idx,len(test_rows),lot_id), flush=True)
                results.append(item)
                continue

            # Создаём отдельную вкладку именно той страницы выдачи, где найден лот.
            ...
            item["page"]=row_page
            print("TEST10D [%s/%s] лот %s: страница %s, создаю временную вкладку" % (idx,len(test_rows),lot_id,row_page), flush=True)

            ws=master_ws
            created=cdp("Target.createTarget",{"url":lot_search_url})
            target_id=created.get("targetId")
            if not target_id:
                raise RuntimeError("Не удалось создать временную вкладку.")

            # Ждём появления target в списке Chrome.
            ...
            while time.time()<deadline:
                ...
                if temp_tab:
                    break
            if not temp_tab:
                raise RuntimeError("Временная вкладка Samruk не появилась.")

            temp_ws=websocket.create_connection(
                temp_tab["webSocketDebuggerUrl"],
                timeout=50,
            ...
            if not wait_search(15):
                raise RuntimeError("Поисковая выдача не загрузилась во временной вкладке.")
            print("TEST10D [%s/%s] лот %s: выдача загружена" % (idx,len(test_rows),lot_id), flush=True)

            # Кликаем нужную карточку только во временной вкладке.
            click_card_js=r"""
            (() => {
              const lot=%s;
              const clean=s=>(s||'').replace(/\s+/g,' ').trim();
              ...
              const els=[...document.querySelectorAll('body *')].filter(e => {
                ...
                return t.includes('№ '+lot) || t.includes('№'+lot);
              });
              ...
              for(let i=0;i<9 && p;i++,p=p.parentElement){
                ...
                if(t.includes(lot) &&
                   /(Осталось:|Стоимость:|Запрос ценовых)/i.test(t) &&
              ...
              card=card || els[0].parentElement || els[0];
              card.scrollIntoView({block:'center'});
              card.dispatchEvent(new MouseEvent('click',{
                bubbles:true,cancelable:true,view:window
              }));
              return {ok:true,text:clean(card.innerText).slice(0,220)};
            })()
            """ % json.dumps(lot_id,ensure_ascii=False)

            v=eval_js(click_card_js)
            if not v.get("ok"):
                raise RuntimeError(v.get("message") or "Не удалось нажать карточку лота.")
            print("TEST10D [%s/%s] лот %s: карточка нажата" % (idx,len(test_rows),lot_id), flush=True)
            ...
            while time.time()<deadline:
                ...
                (() => {
                  ...
                  const body=clean(document.body ? document.body.innerText : '');
                  return {
                    docs:/Документы/i.test(body),
                    lot:/№\s*\d{5,}/.test(body)
                  };
                })()
                """)
                if v.get("docs") and v.get("lot"):
                    popup=True
                    break
            if not popup:
                raise RuntimeError("Popup лота не открылся.")
            print("TEST10D [%s/%s] лот %s: popup открыт" % (idx,len(test_rows),lot_id), flush=True)

            # Раскрываем Документы.
            expand_docs_js=r"""
            (async () => {
              const clean=s=>(s||'').replace(/\s+/g,' ').trim();
              ...
              let a=techLink();
              if(a) return {ok:true,already:true};

              const docs=[...document.querySelectorAll('button,a,div,span')].filter(e=>{
                const t=clean(e.innerText);
                const r=e.getBoundingClientRect();
                return /^Документы(?:\s*\d+)?$/i.test(t) &&
                       r.width>0 && r.height>0;
              });

              if(!docs.length) return {ok:false,message:'Кнопка Документы не найдена'};
              ...
              a=techLink();
              return a ? {ok:true} :
                {ok:false,message:'Техспецификация не появилась после раскрытия Документы'};
            })()
            """
            v=eval_js(expand_docs_js,True)
            if not v.get("ok"):
                raise RuntimeError(v.get("message") or "Не удалось раскрыть документы.")
            print("TEST10D [%s/%s] лот %s: Документы раскрыты, техспека найдена" % (idx,len(test_rows),lot_id), flush=True)
            ...
            (() => {
              ...
              a.click();
              return {ok:true,text:clean(a.innerText)};
            })()
            """
            v=eval_js(click_spec_js)
            if not v.get("ok"):
                raise RuntimeError(v.get("message") or "Не удалось нажать техспецификацию.")
            print("TEST10D [%s/%s] лот %s: клик по техспецификации выполнен, жду PDF" % (idx,len(test_rows),lot_id), flush=True)
            ...
            while time.time()<deadline:
                ...
                fresh=[
                    p for p in ROOT.glob("Lot_*.pdf")
                    if str(p.resolve()) not in before and p.stat().st_mtime >= started-2
                ]
                exact=[p for p in fresh if lot_id in p.name]
                pool=exact or fresh
            ...
            print("TEST10D [%s/%s] лот %s: PDF найден: %s" % (idx,len(test_rows),lot_id,pdf.name), flush=True)

            data=mod.parse(pdf)
            parsed_lot=str(data.get("lot_id") or "").strip()
            if parsed_lot and parsed_lot!=lot_id:
            ...
            print("TEST10D [%s/%s] лот %s: ГОТОВО, PDF -> JSON -> Supabase HTTP %s" % (idx,len(test_rows),lot_id,status), flush=True)
            ...
            print("TEST10D [%s/%s] лот %s: ОШИБКА: %s" % (idx,len(test_rows),lot_id,str(e)), flush=True)

        finally:
            # Закрываем только временную вкладку; исходная выдача остаётся нетронутой.
            try:
        ...
        results.append(item)
        print("TEST10D [%s/%s] лот %s: завершён со статусом %s" % (idx,len(test_rows),lot_id,item.get("status")), flush=True)

    print("TEST10F: пакет завершён. Результатов: %s" % len(results), flush=True)
    try:
        master_ws.close()
    except Exception:
        pass
    ...
    def do_GET(self):
        if self.path.startswith("/diagnose-pages"):
            try:
                ...
                self.headers_ok(500)
                self.wfile.write(json.dumps({"ok":False,"build":"v6.7_TEST10G_BATCH5_COVERAGECHECK","message":str(e)},ensure_ascii=False).encode("utf-8"))
            ...
            try:
                ...
                # TEST10F: на этом контрольном запуске НЕ пересматриваем все старые PDF.
                # Это сокращает время запроса и изолирует проверку именно пакета из 5 новых лотов.
                pending=[]
                print("TEST10F: проверка старых pending PDF пропущена; запускаю только пакет новых лотов.", flush=True)
...
print("ProcureVision TEST10G COVERAGECHECK local bridge: http://127.0.0.1:8765")
