...
def diagnose_samruk_pages():
    ...
    def wait_expected(n,target,previous_lots,timeout=15):
        ...
        while time.time()<deadline:
            ...
            # TEST10K CONTENTGUARD: Samruk SPA может уже показать новый page/диапазон,
    ...
    pages=min(pages,50)  # TEST10K MULTIPAGE: диагностика до 50 страниц.
...
def read_current_samruk():
    """
    TEST10K: сбор всей выдачи Samruk выполняется в ОТДЕЛЬНОЙ временной вкладке.
    ...
    try:
        ...
        def wait_page(n,prev_lots,timeout=18):
            ...
            while time.time()<deadline:
                ...
                # TEST10K CONTENTGUARD: page/"Показано" могут измениться раньше DOM карточек.
        ...
        for pass_no in range(1,4):
            prev_lots=[]
            print("Samruk TEST10K: проход %s/3, сейчас уникальных %s/%s." % (pass_no,len(collected),target), flush=True)
            for n in range(1,pages+1):
                ...
                for attempt in range(1,4):
                    ...
                    if ok: break
                if not ok:
                    print("Samruk TEST10K: страница %s/%s не подтвердилась в проходе %s; продолжаю следующий проход." % (n,pages,pass_no), flush=True)
                    prev_lots=[]
                    continue
                ...
                for c in (snap.get("cards") or []):
                    ...
                    collected[lot]=cc
                prev_lots=current
                print("Samruk TEST10K: проход %s, страница %s/%s, lot_id=%s, уникальных %s/%s." % (
        ...
        print("Samruk TEST10K: собрано %s/%s уникальных строк во временной вкладке." % (len(cards),target), flush=True)
...
def auto_download_and_process_techspecs(rows):
    ...
    print("TEST10K: выбраны лоты: %s" % ", ".join(selected), flush=True)
    ...
    for idx,row in enumerate(test_rows,1):
        ...
        print("TEST10K [%s/%s] лот %s: старт" % (idx,len(test_rows),lot_id), flush=True)
        try:
            ...
            print("TEST10K [%s/%s] лот %s: popup открыт, данные карточки сняты" % (idx,len(test_rows),lot_id), flush=True)
            ...
            if found:
                ...
                print("TEST10K [%s/%s] лот %s: PDF -> JSON -> Supabase HTTP %s" % (idx,len(test_rows),lot_id,status), flush=True)
            else:
                ...
                print("TEST10K [%s/%s] лот %s: отдельной техспеки нет; карточка -> JSON -> Supabase HTTP %s" % (idx,len(test_rows),lot_id,status), flush=True)

        except Exception as e:
            item.update({"ok":False,"status":"error","message":str(e)})
            print("TEST10K [%s/%s] лот %s: ОШИБКА: %s" % (idx,len(test_rows),lot_id,str(e)), flush=True)
        finally:
            try:
        ...
        results.append(item)
        print("TEST10K [%s/%s] лот %s: статус %s" % (idx,len(test_rows),lot_id,item.get("status")), flush=True)

    print("TEST10K: пакет завершён. Результатов: %s" % len(results), flush=True)
    try: master_ws.close()
    except Exception: pass
    return results
    ...
    def do_GET(self):
        if self.path.startswith("/diagnose-pages"):
            try:
                ...
                self.headers_ok(500)
                self.wfile.write(json.dumps({"ok":False,"build":"v6.7_TEST10K_MULTIPAGE","message":str(e)},ensure_ascii=False).encode("utf-8"))
            ...
            try:
                ...
                # TEST10K: на этом контрольном запуске НЕ пересматриваем все старые PDF.
                # Это сокращает время запроса и изолирует проверку именно пакета из 5 новых лотов.
                pending=[]
                print("TEST10K: проверка старых pending PDF пропущена; запускаю только пакет новых лотов.", flush=True)
...
print("ProcureVision TEST10K MULTIPAGE local bridge: http://127.0.0.1:8765")
