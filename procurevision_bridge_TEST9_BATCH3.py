...
def diagnose_samruk_pages():
    ...
    def wait_expected(n,target,previous_lots,timeout=15):
        ...
        while time.time()<deadline:
            ...
            # Лучший признак — корректный диапазон "Показано X-Y".
            # Если сайт его не отдал, принимаем page=n + реально изменившийся набор лотов.
            if lots and (range_ok or (page_ok and changed)):
                return last,True
        return last,False
...
def read_current_samruk():
    """
    TEST9: собирает ВСЕ страницы Samruk с подтверждением реального диапазона
    "Показано X-Y". В каждую карточку записывает номер страницы, чтобы
    последующая обработка техспеки открывала именно ту страницу, где лежит лот.
    """
    tabs=get_tabs()
    tab=choose_samruk_tab(tabs)
    if not tab:
    ...
    def page_snapshot():
        ...
        (() => {
          ...
          for(const x of raw){
            ...
            seen.add(x.lot);
            cards.push({lot:x.lot,text:x.text,href:x.href});
          }
          const u=new URL(location.href);
          const page=u.searchParams.get('page') ||
                     (new URLSearchParams((location.hash.split('?')[1]||''))).get('page') || '';
          return {
            keyword,target,shown_from:shownFrom,shown_to:shownTo,cards,
            loggedIn:!!localStorage.getItem('jhi-authenticationtoken'),
            page:String(page||''),url:location.href
          };
        })()
        """
        res=cdp("Runtime.evaluate",{"expression":js,"returnByValue":True})
    ...
    def wait_expected(n,target,previous_lots,timeout=15):
        ...
        while time.time()<deadline:
            time.sleep(0.55)
            last=page_snapshot()
            lots=[str(x.get("lot") or "") for x in (last.get("cards") or [])]
            shown_from=int(last.get("shown_from") or 0)
            shown_to=int(last.get("shown_to") or 0)
            page=str(last.get("page") or "")
            range_ok=(shown_from==expected_from and (not expected_to or shown_to==expected_to))
    ...
    if not (first.get("cards") or []) and "q=" in ((first.get("url") or tab.get("url") or "").lower()):
        try:
            cdp("Page.reload",{"ignoreCache":True})
    ...
    keyword=first.get("keyword") or ""
    target=int(first.get("target") or 0)
    current_url=first.get("url") or tab.get("url") or ""
    if not target:
        ws.close()
    ...
    try:
        for n in range(1,pages+1):
            url=make_page_url(original_url,n)
            cdp("Page.navigate",{"url":url})
            snap,ok=wait_expected(n,target,previous_lots,15)
            ...
            if not ok:
                ...
                snap,ok=wait_expected(n,target,previous_lots,12)
            ...
            if not ok:
                raise RuntimeError("Samruk: не подтвердилась страница %s из %s." % (n,pages))

            current_cards=snap.get("cards") or []
            current_lots=[]
            for c in current_cards:
                lot=str(c.get("lot") or "").strip()
                if not lot:
                    m=re.search(r"№\s*(\d{5,})",c.get("text","") or "")
                    lot=m.group(1) if m else ""
                if not lot:
                    continue
                ...
                if lot not in collected:
                    collected[lot]=cc

            previous_lots=current_lots
            print("Samruk TEST9: страница %s/%s, на странице %s, всего уникальных %s." % (
                n,pages,len(current_lots),len(collected)))
    finally:
        try:
            cdp("Page.navigate",{"url":original_url})
            time.sleep(1.0)
    ...
    print("Samruk TEST9: собрано %s/%s строк." % (len(cards),target))
...
def auto_download_and_process_techspecs(rows):
    """
    v6.7_TEST9_BATCH3:
    До 3 лотов обрабатываются в отдельных временных вкладках Samruk.
    ...
    # TEST9: контрольный пакет — до 3 лотов без raw.techspec по всей собранной выдаче.
    candidates=rows
    test_rows=[]
    for r in candidates:
    ...
    def do_GET(self):
        if self.path.startswith("/diagnose-pages"):
            try:
                ...
                self.headers_ok(500)
                self.wfile.write(json.dumps({"ok":False,"build":"v6.7_TEST9_BATCH3","message":str(e)},ensure_ascii=False).encode("utf-8"))
            ...
            try:
                ...
                # Сначала подхватываем уже скачанные вручную PDF, если такие остались.
                pending=process_pending_techspecs()
...
print("ProcureVision local bridge: http://127.0.0.1:8765")
